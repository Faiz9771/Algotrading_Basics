...
def renko_DF(DF):
    ...
    # stocktrends API changed; try both method names with a safe fallback
    try:
        renko_df = df2.get_bricks()
    except AttributeError:
        if hasattr(df2, "get_ohlc_data"):
...
for ticker in tickers:
    ...
    def _normalize_col(col_name: str) -> str:
        name = str(col_name)
        # remove trailing _<TICKER> if present (e.g., Close_MSFT)
        suffix = f"_{ticker}"
    ...
    try:
        ...
        # if tz conversion fails, still proceed with raw intraday data
        pass
...
for ticker in tickers:
    ...
    if df[ticker].empty:
        print(f"{ticker}: no data to build renko, skipping.")
        continue
    renko = renko_DF(df[ticker])
    if renko.empty:
        print(f"{ticker}: insufficient data for ATR/renko, skipping.")
        continue
...
# use only tickers with built renko data going forward
