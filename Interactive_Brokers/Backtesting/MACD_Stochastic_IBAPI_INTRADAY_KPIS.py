...
tickers = ["META","AMZN","INTC","MSFT","AAPL","GOOG","CSCO","CMCSA","ADBE","NVDA",
...
for ticker in tickers:
        ...
        if tickers_signal[ticker] == "":
            ...
            if ohlc_dict[ticker]["macd"].iloc[i]> ohlc_dict[ticker]["signal"].iloc[i] and \
               ...
               ohlc_dict[ticker]["stoch"].iloc[i] > ohlc_dict[ticker]["stoch"].iloc[i-1]:
                   ...
                   trade_data[ticker][trade_count[ticker]] = [ohlc_dict[ticker]["Close"].iloc[i]]
                     
        elif tickers_signal[ticker] == "Buy":
            if ohlc_dict[ticker]["Low"].iloc[i]<ohlc_dict[ticker]["Close"].iloc[i-1] - ohlc_dict[ticker]["atr"].iloc[i-1]:
                tickers_signal[ticker] = ""
                trade_data[ticker][trade_count[ticker]].append(ohlc_dict[ticker]["Close"].iloc[i-1] - ohlc_dict[ticker]["atr"].iloc[i-1])
                trade_count[ticker]+=1
    ...
    if trade_count[ticker]%2 != 0:
        trade_data[ticker][trade_count[ticker]].append(ohlc_dict[ticker]["Close"].iloc[i])
